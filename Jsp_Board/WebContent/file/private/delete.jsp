...
<%@page import="jsp.file.FileDao"%>
<%@page import="jsp.file.FileDto"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
 	//1. GET 방식 전달되는 삭제할 파일 번호를 읽어온다.
 	int num=Integer.parseInt(request.getParameter("num"));
 	//2. DB 에서 해당 파일의 정보를 읽어돈다
 	FileDto dto=FileDao.getInstance().getData(num);
 	
 	//만일 로그인된 아이디와 글 작성자가 다르면 에러를 응답한다.
 	String id=(String)session.getAttribute("id"); //로그인된 아이디
 	if(!dto.getWriter().equals(id)){
 		response.sendError(HttpServletResponse.SC_FORBIDDEN, "남의 파일 지우면 혼난당!");//forbidden 은 403
 		return; //메소드 종료
 	}
 	...
 	/*
 		...
 		File.separtor 는 운영체제에 맞게끔 알맞은 파일 구분자를 얻어낼 수 있다.
 	*/
 	...
 	response.sendRedirect(cPath+"/file/list");
 	//리다일렉트 응답이기에 html은 필요없다
